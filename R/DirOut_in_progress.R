DirOut=function(data, DirOutmatrix = FALSE, depth.dir="RP", D_value=TRUE,quantile.used=floor((n + p + 1)/2),...)
{
  ...
  if (length(temp)==2)
  {
    ...
    #p=dim(data)[1]
    #n=dim(data)[2]
    #Dirout=matrix(0,n,p) # create a 0 matrix of size nxp
    #dmat=matrix(0,p,n) # create a 0 matrix of pxn
    if ((depth.dir=="MhD")||(depth.dir=="RP")){
      medvec = apply(data,1,median) # vector of size p
      madvec=apply(data,1,mad)
      Dirout = t((data-medvec)/(madvec))
      #outmat=abs((data-medvec)/(madvec))
      #signmat=sign((data-medvec))
      #Dirout=t(outmat*signmat)
    }
    if ((depth.dir=="SD")||(depth.dir=="HS")) ## the same as previous condition
    {
