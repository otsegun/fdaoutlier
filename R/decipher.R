...
testdt <-model1(n = n, p = p, out.rate = alpha, plotdt = T)
dt <- testdt$data

#res <- msplot(data = dt, plot = T, dirout = T)

### 
data <- dt[1:10, 1:5]
msplot <- function(data, depth.dir = "RP", plot = TRUE, col.normal = "blue", col.out = "red",
                   col = "grey", dirout = FALSE, plot.type = "scatter", ...){
  ...
  if (length(temp) == 2)
  {
    ...
    if (dirout)
    {
      ...
      if (plot)
      {
        ...
        p <-
          ggplot(data = ms.data, aes(x = x, y = y)) + geom_point(col = col.point) +
          geom_path(
            data = elip.data,
            aes(x = x, y = y),
            show.legend = FALSE,
    ...
    {
      mo <- result$out_avr
      xlim = c(min(mo) - 0.1 * (max(mo) - min(mo)), max(mo) + 0.1 * (max(mo) -
                                                                       min(mo)))
      vo <- result$out_var
      fo <- mo ^ 2 + vo
      ms.data = data.frame(x = mo, y = vo, col = col)
      p <-
        ggplot(data = ms.data, aes(x, y)) + geom_point(colour = col, show.legend =
                                                         FALSE) +
        xlab("MO") + ylab("VO") + labs(title = "MS-Plot") + theme(plot.title = element_text(hjust = 0.5))
      if (plot)
      {
  ...
  if (length(temp) == 3)
  {
    ...
    if (d == 2)
    {
      ...
      if (!dirout)
      {
        ...
        xlim = c(min(mo1) - 0.1 * (max(mo1) - min(mo1)), max(mo1) + 0.1 *
                   (max(mo1) - min(mo1)))
        ylim = c(min(mo2) - 0.1 * (max(mo2) - min(mo2)), max(mo2) + 0.1 *
                   (max(mo2) - min(mo2)))
        vo <- result$out_var
        fo <- mo1 ^ 2 + mo2 ^ 2 + vo
      ...
      if (dirout)
      {
        ...
        if (plot)
        {
          ...
          if (plot.type == "scatter")
          {
            ...
            ms.data = data.frame(x = MO,
                                 y = vo,
                                 col = col.point)
            p <-
              ggplot(data = ms.data, aes(x, y)) + geom_point(aes(colour = col), show.legend =
                                                               FALSE) +
